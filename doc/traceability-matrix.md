...
| FR-ERR-3 | Validation errors before any provider call | `search/translate`, all capabilities (target/recipient/change validation), `index` (zod) | `translate.test`, capability tests | ✅ (read, write, and organise all validate before any Graph call) |
...
| NFR-OPS-4 | MCP behavioural annotations; send/organise destructive | `index` (tool registration) | `toolsAnnotations.test` | ✅ (all 8 tools annotated; send + organize_mail destructive) |
...
> **Live organise validation** (criterion 2 for C6–C8) will confirm category vs. folder listing,
> category/folder creation, and the category-PATCH / `move` fan-out (including the conversation
> union) against a real mailbox.
